...
export const $$: A.analysePath = () => {
    return ($) => {
        ...
        type State =
            | ["error", g_this.T.PathError]
            | ["processing", {
                currentDirectory: g_this.T.Directory
            }]
        ...
        $.filePath.directoryPath.__forEach(($) => {
            const stepID = $
            switch (state[0]) {
                case 'error':
                    pl.cc(state[1], ($) => {
            ...
            case 'processing':
                return pl.cc(state[1], ($) => {
                    ...
                    switch (ps.currentDirectory.type[0]) {
                                    ...
                                    switch ($[0]) {
                                        ...
                                        case 'set':
                                            return pl.cc($[1], ($) => {
                                                ...
                                                return x.extensions.__getEntry<g_this.T.AnalysisResult>(
                                                    $,
                                                    () => {
                                                        return ["success", {
                        ...
                        case 'type':
                            return pl.cc(ps.currentDirectory.type[1], ($) => {
                                return $.nodes.__getEntry(
                                    fileNameWithExtension,
                                    ($): g_this.T.AnalysisResult => {
                                        switch ($.type[0]) {
                                            case 'file':
                                                return pl.cc($.type[1], ($) => {
                                            ...
                                            case 'directory':
                                                return pl.cc($.type[1], ($): g_this.T.AnalysisResult => {
                                                    return ["error", {
